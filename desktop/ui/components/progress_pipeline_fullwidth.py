"""
Visual pipeline progress component - guaranteed full width version
"""
...
class ProgressPipeline:
    """Visual representation of generation pipeline progress - full width guaranteed"""
    ...
    def render(self, container: Optional[ui.element] = None):
        """Render the progress pipeline - directly in container without card"""
        # Store the container reference
        self.container = container or ui.column()
        
        with self.container:
            # Skip the card and render directly
            with ui.column().classes('w-full').style('background-color: #1F1F1F; border: 1px solid #333333; border-radius: 8px; padding: 16px; width: 100% !important;'):
                # Header - full width
                ...
                with ui.column().classes('w-full mb-3'):
                    # Progress bar with explicit full width
                    self.progress_bar = ui.linear_progress(value=0, show_value=False)
                    self.progress_bar.classes('w-full')
                    self.progress_bar.style('height: 24px !important; width: 100% !important; border-radius: 12px;')
                ...
                # Steps container - full width without nested containers
    ...
    def _render_step(self, step: PipelineStep, index: int) -> ui.element:
        ...
        with ui.row().classes('w-full items-center').style('padding: 8px 0;') as step_element:
            # Step icon
            with ui.element('div').classes('w-10 h-10 rounded-full bg-gray-700 flex items-center justify-center flex-shrink-0'):
                if not hasattr(self, 'step_icons'):
                    self.step_icons = {}
                self.step_icons[step.id] = ui.icon(step.icon, size='sm').classes('text-gray-400')
                
            # Step info - flex-grow for remaining width
            ...
            self.step_status_icons[step.id] = ui.icon('', size='lg').classes('hidden')
    ...
    def start(self):
        ...
        for step_id, element in self.step_elements.items():
            ...
            self.step_icons[step_id].classes('text-gray-400', remove='text-green-500 text-blue-500')
