"""
Enhancement panel with guaranteed full-width dropdowns
"""
...
class EnhancementPanel:
    """Panel for prompt enhancement fields with full-width dropdowns"""
    ...
    def render(self, container: Optional[ui.element] = None):
        ...
        with container or ui.column():
            ...
            # Fields container - ensure full width propagation
            self.fields_container = ui.column().classes('w-full gap-3')
            self._render_fields()
    ...
    def _render_field(self, field_id: str, config: Dict[str, Any]):
        """Render a single enhancement field with guaranteed full width"""
        ...
        if field_type == 'multi_checkbox':
            ...
            with expansion:
                ...
                with ui.grid(columns=1).classes('w-full gap-2 p-2'):
                    for option_id, option_label in options.items():
                        checkbox = ui.checkbox(text=option_label).props('dense')
            ...
            # Dropdown field - ensure full width
            options = config.get('options', {})
            if options:
                # Create select with explicit full width
                select = ui.select(
                    label=config['label'],
                    options={k: v for k, v in options.items()},  # Use dict for key-value pairs
                    value=None,
                    with_input=True
                )
                
                # Apply classes and styles for full width
