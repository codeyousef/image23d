...
"""
            ...
            # Initial updates - Group components don't support .load()
            # Add refresh button for manual updates
            refresh_creds_btn = gr.Button("🔄 Refresh Credentials", variant="secondary")
            refresh_creds_btn.click(
                update_stored_services,
                outputs=[stored_services]
            )
            
            if service_choices:
                # Use dropdown change event instead of .load()
                service_dropdown.change(
                    update_service_info,
                    inputs=[service_dropdown],
                    outputs=[service_info]
