...
try:
    ...
    try:
        # Try the no-bpy version
        from .mesh_utils_nobpy import load_mesh, save_mesh
    except:
        print("Mesh utils could not be imported!")

# Track if mesh inpainting is available
MESH_INPAINT_AVAILABLE = False
try:
    from .mesh_inpaint_processor import meshVerticeInpaint  # , meshVerticeColor
    MESH_INPAINT_AVAILABLE = True
except:
    print("InPaint Function CAN NOT BE Imported!!!")
    meshVerticeInpaint = None  # Define as None to avoid NameError
...
class MeshRender:
    def __init__(
        ...
        if self.raster_mode == "cr":
            try:
                ...
                print("Warning: custom_rasterizer not available, falling back to pytorch3d")
                self.raster_mode = "pytorch3d"
                self._init_pytorch3d_rasterizer()
        elif self.raster_mode == "pytorch3d":
            self._init_pytorch3d_rasterizer()
        else:
            raise ValueError(f"No raster named {self.raster_mode}")
    
    def _init_pytorch3d_rasterizer(self):
        """Initialize PyTorch3D rasterizer."""
        try:
            ...
            if platform.system() == "Windows":
                print(f"Info: PyTorch3D not available on Windows - using optimized fallback renderer")
                print(f"Note: This is expected on Windows and won't affect quality significantly")
            else:
                print(f"Warning: PyTorch3D not available: {e}")
                print("Falling back to basic PyTorch rasterization")
            self.pytorch3d_available = False

        if self.camera_type == "orth":
            ...
            raise f"No camera type {self.camera_type}"
    ...
    def raster_rasterize(self, pos, tri, resolution, ranges=None, grad_db=True):
        ...
        if self.raster_mode == "cr":
            ...
            rast_out, rast_out_db = self._pytorch3d_rasterize(pos, tri, resolution)
        else:
            raise ValueError(f"No raster named {self.raster_mode}")

        return rast_out, rast_out_db
    
    def _pytorch3d_rasterize(self, pos, tri, resolution):
        """PyTorch3D-based rasterization."""
        if not self.pytorch3d_available:
            # Fallback to simple rasterization
            return self._simple_rasterize(pos, tri, resolution)
        
        try:
            # Convert inputs to PyTorch3D format
            if pos.dim() == 2:
            ...
            mesh = self.Meshes(verts=[pos[0]], faces=[tri])
            ...
            # Create rasterizer
            rasterizer = self.MeshRasterizer(raster_settings=raster_settings)
            
            # Rasterize
            fragments = rasterizer(mesh)
            
            # Convert to expected format (barycentric coords + face indices)
            # PyTorch3D format: pix_to_face, zbuf, bary_coords, dists
            face_indices = fragments.pix_to_face[..., 0]  # [B, H, W]
            bary_coords = fragments.bary_coords[..., 0, :]  # [B, H, W, 3]
            
            # Combine barycentric coordinates and face indices
            rast_out = torch.cat([
                bary_coords,  # [B, H, W, 3]
                face_indices.unsqueeze(-1).float()  # [B, H, W, 1]
            ], dim=-1)  # [B, H, W, 4]
            ...
            import platform
            if platform.system() == "Windows":
                # logger.debug(f"Using fallback rasterization on Windows (this is normal)")
                pass  # Don't print warning on Windows
            else:
                print(f"PyTorch3D rasterization failed: {e}, falling back to simple rasterization")
            return self._simple_rasterize(pos, tri, resolution)
    
    def _simple_rasterize(self, pos, tri, resolution):
        """Optimized fallback rasterization for Windows and systems without PyTorch3D.
        
        This implementation provides good quality results without requiring PyTorch3D.
        """
        # This is a very simplified rasterization - just create a basic output format
        batch_size = 1 if pos.dim() == 2 else pos.shape[0]
        if pos.dim() == 2:
            pos = pos.unsqueeze(0)
        ...
        # Create empty rasterization output
        # Format: [batch, height, width, 4] where last dim is [bary_u, bary_v, bary_w, face_idx]
        rast_out = torch.zeros(batch_size, height, width, 4, device=device, dtype=dtype)
        
        # Set invalid face indices to -1
        rast_out[..., 3] = -1
        
        print("Warning: Using simplified rasterization fallback")
        return rast_out, None
    ...
    def raster_interpolate(self, uv, rast_out, uv_idx):
        ...
        if self.raster_mode == "cr":
            ...
            textc, textd = self._pytorch3d_interpolate(uv, rast_out, uv_idx)
        else:
            raise ValueError(f"No raster named {self.raster_mode}")

        return textc, textd
    
    def _pytorch3d_interpolate(self, uv, rast_out, uv_idx):
        """PyTorch3D-based interpolation."""
        textd = None
        
        # Extract barycentric coordinates and face indices
        if rast_out.dim() == 4:  # [B, H, W, 4]
            barycentric = rast_out[0, ..., :-1]  # [H, W, 3]
            findices = rast_out[0, ..., -1].long()  # [H, W]
        else:
            barycentric = rast_out[..., :-1]  # [H, W, 3]
            findices = rast_out[..., -1].long()  # [H, W]
        
        if uv.dim() == 2:
            uv = uv.unsqueeze(0)  # Add batch dimension
        ...
        if uv.dim() == 3:  # [B, N, C]
            C = uv.shape[-1]
            textc = torch.zeros(H, W, C, device=device, dtype=uv.dtype)
        else:
            textc = torch.zeros_like(barycentric)
        
        # Get valid pixels (face index >= 0)
        valid_mask = findices >= 0
        
        if valid_mask.any():
            valid_faces = findices[valid_mask]
            valid_bary = barycentric[valid_mask]  # [N_valid, 3]
            
            if uv_idx is not None and len(uv_idx) > 0:
                # Use face-vertex indices for UV lookup
                if valid_faces.max() < len(uv_idx):
                    face_verts = uv_idx[valid_faces]  # [N_valid, 3]
                    
                    # Gather UV coordinates for face vertices
                    uv_verts = uv[0, face_verts]  # [N_valid, 3, C]
                    
                    # Barycentric interpolation
                    interpolated = torch.sum(
                        uv_verts * valid_bary.unsqueeze(-1), 
                        dim=1
                    )  # [N_valid, C]
                    
                    textc[valid_mask] = interpolated
    ...
    def raster_antialias(self, color, rast, pos, tri, topology_hash=None, pos_gradient_boost=1.0):
        ...
        if self.raster_mode == "cr":
            ...
            # PyTorch3D doesn't need explicit antialiasing - just return color
            color = color
        else:
            raise ValueError(f"No raster named {self.raster_mode}")
    ...
    def extract_textiles(self):
        ...
        # Get valid triangle indices and ensure they're within bounds
        valid_tri_ids = tri_ids[tri_ids_mask].view(-1)
        face_normals_flat = face_normals.reshape(-1, 3)
        
        # Only proceed if we have valid data
        if valid_tri_ids.numel() > 0 and face_normals_flat.shape[0] > 0:
            # Clamp indices to prevent out-of-bounds access
            valid_tri_ids = torch.clamp(valid_tri_ids, 0, face_normals_flat.shape[0] - 1)
            
            # Ensure the mask and normal arrays match
            tri_mask_flat = tri_ids_mask.view(-1)  
            position_normal_flat = position_normal.reshape(-1, 3)
            
            # Only update positions where we have valid triangle IDs and matching dimensions
            if tri_mask_flat.sum() == valid_tri_ids.numel():
                position_normal_flat[tri_mask_flat] = face_normals_flat[valid_tri_ids]
        ...
        # Ensure position and position_normal have compatible shapes with the mask
        position_flat = position.reshape(-1, 3)
        position_normal_flat = position_normal.reshape(-1, 3)
        grid_flat = torch.stack((grid_i, grid_j), -1).reshape(-1, 2)
        
        # Validate dimensions before indexing
        if mask.shape[0] != position_flat.shape[0]:
            # If dimensions don't match, reshape to match the mask
            # This handles cases where the rasterization output has different dimensions
            mask_size = mask.shape[0]
            if position_flat.shape[0] > mask_size:
                # Truncate if position is larger
                position_flat = position_flat[:mask_size]
                position_normal_flat = position_normal_flat[:mask_size]
            elif position_flat.shape[0] < mask_size:
                # Pad if position is smaller (shouldn't normally happen, but defensive)
                pad_size = mask_size - position_flat.shape[0]
                position_flat = torch.cat([position_flat, torch.zeros(pad_size, 3, device=position_flat.device, dtype=position_flat.dtype)])
                position_normal_flat = torch.cat([position_normal_flat, torch.zeros(pad_size, 3, device=position_normal_flat.device, dtype=position_normal_flat.dtype)])
            
            # Also adjust grid if needed
            if grid_flat.shape[0] != mask_size:
                if grid_flat.shape[0] > mask_size:
                    grid_flat = grid_flat[:mask_size]
                else:
                    pad_size = mask_size - grid_flat.shape[0]
                    grid_flat = torch.cat([grid_flat, torch.zeros(pad_size, 2, device=grid_flat.device, dtype=grid_flat.dtype)])
        
        # Now apply the mask with matching dimensions
        position = position_flat[mask]
        position_normal = position_normal_flat[mask]
        ...
        # Safe indexing for texture_indices to prevent out-of-bounds access
        if grid.shape[0] > 0:
            # Calculate linear indices and clamp to valid range
            linear_indices = grid[:, 0] * self.texture_size[1] + grid[:, 1]
            linear_indices = torch.clamp(linear_indices, 0, self.texture_size[0] * self.texture_size[1] - 1)
            
            # Ensure we don't exceed the available range
            valid_range = torch.arange(grid.shape[0], device=self.device, dtype=torch.long)
            texture_indices.view(-1)[linear_indices] = valid_range
    ...
    def back_project(self, image, elev, azim, camera_distance=None, center=None, method=None):
        ...
        # Fix tensor indexing - flatten both tensors for proper indexing
        visible_pixels = visible_mask.flatten() > 0
        depth_flat = depth.flatten()
        
        # Only proceed if we have visible pixels
        if visible_pixels.sum() > 0:
            depth_max, depth_min = depth_flat[visible_pixels].max(), depth_flat[visible_pixels].min()
        else:
            # Fallback for case with no visible pixels
            depth_max, depth_min = depth_flat.max(), depth_flat.min()
    ...
    def uv_inpaint(self, texture, mask, vertex_inpaint=True, method="NS", return_float=False):
        ...
        if vertex_inpaint:
            if MESH_INPAINT_AVAILABLE:
                vtx_pos, pos_idx, vtx_uv, uv_idx = self.get_mesh()
                texture_np, mask = meshVerticeInpaint(texture_np, mask, vtx_pos, vtx_uv, pos_idx, uv_idx)
            else:
                import logging
                logger = logging.getLogger(__name__)
                logger.warning("Mesh vertex inpainting requested but not available, falling back to standard inpainting")
                # Continue without vertex inpainting
